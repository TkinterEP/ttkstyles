...
# Package
from ttkstyles.exceptions import TtkStyleException
from ttkstyles.gtk.generator import ThemeGenerator
from ttkstyles.utils import chdir
...
class GtkThemeError(TtkStyleException):
    pass
...
class GtkThemeGenerator(ThemeGenerator):
    ...
    def _parse_theme(self):
        ...
        with chdir(os.path.dirname(theme_file)):
    ...
    def _parse_lines_generic(self, lines: List[str]):
        ...
        images = list(filter(lambda d: "function" in d, images))
    ...
    def parse_unreferenced_images(self, images: List[dict]):
        ...
        for image in images:
            ...
            if image.get("function", None) not in self.GTK_FUNCTIONS:
                print("[Generator.parse_unreferenced_images] Unknown function: {}".format(image.get("function", None)))
                continue
    ...
    def process_image_dict(self, widget: str, image: dict) -> Tuple[str, Tuple, Tuple, dict]:
        ...
        while "" in state:
            state.remove("")
        print("Parsed {} from {} for {}".format(state, image, image["file"]))
